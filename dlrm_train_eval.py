...
def get_args(print_args=True):
    ...
    parser.add_argument("--embedding_vec_size", type=int, default=32)
    parser.add_argument(
    ...
    parser.add_argument("--bottom_mlp", type=int_list, default="128,64,32")
    parser.add_argument("--top_mlp", type=int_list, default="128,128,64,32")
    parser.add_argument(
    ...
    parser.add_argument(
        "--eval_batches", type=int, default=100, help="number of eval batches"
    )
    ...
    parser.add_argument("--decay_batches", type=int, default=500)
    parser.add_argument("--decay_start", type=int, default=200)
    parser.add_argument("--train_batches", type=int, default=200)
    parser.add_argument("--loss_print_interval", type=int, default=10)
    parser.add_argument(
        "--table_size_array",
        type=int_list,
...
def trace_handler(prof):
    if not os.path.exists("./traces/64"):
        os.makedirs("./traces/64")
    prof.export_chrome_trace(os.path.join("./traces/64", f"dlrm_trace_step{prof.step_num}.json"))
    ...
    with make_criteo_dataloader(
        ...
        with torch.profiler.profile(
            ...
            for step in range(1, args.train_batches + 1):
                ...
                with torch.profiler.record_function(f"train_rank_{rank}_step_{step}"):
                ...
                if args.eval_interval > 0 and step % args.eval_interval == 0:
                    with torch.profiler.record_function(f"eval_rank_{rank}_step_{step}"):
